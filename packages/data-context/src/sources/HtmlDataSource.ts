...
export class HtmlDataSource {
  ...
  async makeServeConfig () {
    ...
    // If mitigating secrets via setting disallowCypressEnv, we need to delete the env object and avoid serializing it completely to the browser
    if (cfg.disallowCypressEnv) {
      delete cfg.env
    }
