...
export const AllCypressErrors = {
  ...
  CYPRESS_ENV_DEPRECATION: () => {
    return errTemplate`\
      ...
      Cypress recommends migrating to the ${fmt.highlight('cy.env()')} command and enabling ${fmt.highlight('disallowCypressEnv')} within your Cypress configuration.
      
      The use of ${fmt.highlight('Cypress.env()')} will warn and throw an error when ${fmt.highlight('disallowCypressEnv')} is explicitly set to true.
    
      Read our ${fmt.highlight('Migration Guide')} for the ${fmt.highlight('disallowCypressEnv')} configuration option, why ${fmt.highlight('Cypress.env()')} is deprecated, and how to migrate to ${fmt.highlight('cy.env()')}: https://on.cypress.io/cypress-env-migration.
    `
  },
