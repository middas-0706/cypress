...
class $Cypress {
  ...
  configure (config: Record<string, any> = {}) {
    ...
    const isDisallowCypressEnvEnabled = this.config('disallowCypressEnv')

    const failCypressEnvWithWarning = () => {
      const err = $errUtils.errByPath('config.disallow_cypress_env')
    ...
    this.env = isDisallowCypressEnvEnabled ? failCypressEnvWithWarning : $SetterGetter.create(env)
