...
export default {
  ...
  config: {
    ...
    invalid_test_override_with_disallow_cypress_env: {
      message: `overriding environment variables via suite or test configuration is not allowed when \`disallowCypressEnv\` is set to \`true\`.`,
      docsUrl: 'https://on.cypress.io/cypress-env-migration',
    },
    disallow_cypress_env: {
      message: `\`Cypress.env()\` does not work when \`disallowCypressEnv\` is set to \`true\`. Please migrate to \`cy.env()\` or leverage other stateful methods to manage variables.`,
      docsUrl: 'https://on.cypress.io/cypress-env-migration',
    },
  },
